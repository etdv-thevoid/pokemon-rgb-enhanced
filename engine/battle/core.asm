...
.PriorityMoveUsed
	...
	cp EXTREMESPEED
	jr z, .compareSpeed  ; if both used Priority Move
	cp QUICK_ATTACK
	jr z, .compareSpeed  ; if both used Priority Move
	jp .playerMovesFirst ; if player used Priority Move and enemy didn't
.playerDidNotUsePriorityMove
	ld a, [wEnemySelectedMove]
	cp EXTREMESPEED
	jr z, .enemyMovesFirst  ; if enemy used Priority Move and player didn't
	cp QUICK_ATTACK
	jr z, .enemyMovesFirst  ; if enemy used Priority Move and player didn't
	ld a, [wPlayerSelectedMove]
	cp COUNTER
...
.notDisabled
	...
	and $3f
	ld [wcd6d], a
; print TYPE:<type>, <power>P/<accuracy>%, and <curPP>/<maxPP>
	call PrintPPText
	call PrintMoveInfoText
	coord hl, 7, 11
	ld [hl], "/"
	coord hl, 5, 11
	ld de, wcd6d
...
.FrozenCheck
	...
	; Adding check for Fire Spin to defrost
	ld a, [wPlayerSelectedMove]
	cp FIRE_SPIN
	jr z, .defrostMon
...
.MonHurtItselfOrFullyParalysed
	ld hl, wPlayerBattleStatus2
	res UsingRage, [hl] ; mon no longer raging
	ld hl,wPlayerBattleStatus1
	ld a,[hl]
...
.gotTextToPrint
	...
	; since the move missed, wDamage will always contain 0 at this point.
	...
	; So, instead we do 1/2 of max health into wDamage by copying the
	; Super Fang Effect code with Max HP variable

	; First we have to determine whose turn it is
	ld a, [H_WHOSETURN]
	and a
	jr nz, .enemyJumpKick

	ld hl,wBattleMonMaxHP
	jr .calculateJumpKickRecoil

.enemyJumpKick
	ld hl,wEnemyMonMaxHP

.calculateJumpKickRecoil
	ld de,wDamage
	ld a,[hli]
	srl a
	ld [de],a
	...
	or b
	jr nz, .applyRecoil
; make sure damage is always at least 1
	ld a,$01
	ld [de],a
.applyRecoil
...
.calcCriticalHitProbability
	...
	sla b                        ; effective (base speed) DEFAULT VALUE
	jr nc, .noCarry
	ld b, $ff                    ; cap at 255/256
.noCarry
	ld a, [de]                   ; read BattleStatus2
	bit GettingPumped, a         ; test for focus energy
	jr z, .noFocusEnergyUsed
.focusEnergyUsed
	sla b                        ; effective (base speed*2) DEFAULT VALUE WITH FOCUS ENERGY
...
.Loop
	...
	srl b                        ; effective (base speed/2) NO FOCUS ENERGY. effective (base speed) WITH FOCUS ENERGY
	jr .SkipHighCritical         ; continue as a normal move
.HighCritical
	sla b                        ; *2 for high critical hit moves
	jr nc, .SkipHighCritical     ; effective (base speed*2) NO FOCUS ENERGY. effective (base speed*4) WITH FOCUS ENERGY
	ld b, $ff                    ; cap at 255/256
.SkipHighCritical
...
; else if the random number generated is greater than or equal to the scaled accuracy, the move misses
	...
	cp b
	jr nc,.moveMissed
; From here until .moveMissed has been rewritten to correctly clear HyperBeam and Rage
; only when the opposite mon is Using a Trapping move and the move successfully hits
; If the UsingTrappingMove bit was set (done by Trapping Move Effect routine),
; that means we are using a Trapping move such as Bind or Wrap.
; If the move hits, we reset HyperBeam and Rage bits for target mon
	ld a,[H_WHOSETURN]
	and a
	jr z,.playerTurn2
.enemyTurn2
	...
	jr z, .moveSuccessfullyHit
	call ClearHyperBeam
	call ClearRage
	jr .moveSuccessfullyHit
.playerTurn2
	ld hl,wPlayerBattleStatus1
	bit UsingTrappingMove,[hl]
	jr z, .moveSuccessfullyHit
...
.moveSuccessfullyHit
	ret
; Continue with default move missed behavior
.moveMissed
	xor a
...
.checkIfFrozen
	bit FRZ, [hl]
	jr z, .checkIfTrapped
	; Add check for Fire Spin
	ld a, [wEnemySelectedMove]
	cp FIRE_SPIN
	jr z, .defrostEnemyMon
	; Add chance to defrost naturally
	call BattleRandom
	cp $34
	jr c, .defrostEnemyMon
...
.defrostEnemyMon ; New routine to thaw mon
	ld hl, wEnemyMonStatus
	res FRZ, [hl]
	xor a
	ld [H_WHOSETURN],a
...
;-------------------------------------------------------
; THIS HAS BEEN REMOVED BECAUSE IT IS BROKEN AND UNFAIR
;-------------------------------------------------------
	ret
...
.putMonToSleep
	...
	ld a, [H_WHOSETURN]
	and a
	jp z, .setSleepCounter
	ld bc, wPlayerBattleStatus2
.setSleepCounter
; reset HyperBeam and Rage if we make them sleep
	ld a, [bc]
	res NeedsToRecharge, a ; target no longer needs to recharge
	res UsingRage, a ; target no locked in rage
	ld [bc], a
...
TriAttackEffect:
; 1/4 chance of inflicting FRZ, BRN, PAR, or doing nothing
; Then has %30 chance of inflicting the chosen one
; Mathematically, this means a 7.5% chance of each one
	call BattleRandom
	cp $40
	jr c, .freeze
	cp $80
...
FreezeBurnParalyzeEffect:
	...
	ld b, a
	ld a, NORMAL
	cp b ; is this a normal move, such as Body Slam or Tri Attack?
	jr z, .next1
	ld a, [wEnemyMonType1]
...
.next1
	...
	jr c, .next2 ; branch ahead if this is a 10% chance effect..
...
.freeze
	...
	call ClearRage ; resets rage condition
	ld a, 1 << FRZ
	ld [wEnemyMonStatus], a
	ld a, ANIM_A9
	call PlayBattleAnimation
...
; These where probably added given that a stat-up move affecting speed or attack will override
; the stat penalties from paralysis and burn respectively.
; But they are always called regardless of the stat affected by the stat-up move.
...
.trappingEffect
	bit UsingTrappingMove, [hl]
	ret nz
	; These have been moved to MoveHitTest routine
	; call ClearHyperBeam ; since this effect is called before testing whether the move will hit,
                        ; the target won't need to recharge even if the trapping move missed
	; call ClearRage ; resets rage condition, same as above
