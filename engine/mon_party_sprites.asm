...
.animateSprite
	...
	jr z, .editCoords
	cp $40 ; tile ID for SPRITE_SNORLAX_M
	jr nz, .editTileIDS
; SPRITE_BALL_M, SPRITE_HELIX, SPRITE_SNORLAX_M and only shake up and down
.editCoords
	dec hl
...
MonPartySpritePointers:
	...
	dw StarterSprite + $100
	db $40 / $10 ; $40 bytes
	db BANK(StarterSprite)
	dw vSprites + $6c0

	dw StarterSprite + $140
	db $40 / $10 ; $40 bytes
	db BANK(StarterSprite)
	dw vSprites + $700
...
; make a copy at wMonPartySpritesSavedOAM.
	...
	cp SPRITE_HELIX << 2
	jr z, .asymmetric
	cp SPRITE_BULBASAUR << 2
	jr z, .asymmetric
	cp SPRITE_CHARMANDER << 2
	jr z, .asymmetric
	cp SPRITE_SNORLAX_M << 2
	jr z, .asymmetric
